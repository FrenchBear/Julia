...
The provided RNGs can generate uniform random numbers of the following types: Float16, Float32, Float64, BigFloat, Bool, Int8, UInt8, Int16, UInt16, Int32, UInt32, Int64, UInt64, Int128, UInt128, BigInt (or complex numbers of those types). Random floating point numbers are generated uniformly in 
[
0
,
1
...
[0,1) for floating point numbers and to 
[
0
,
1
...
[0,1)+i[0,1) for complex floating point numbers;
...
rand(MersenneTwister(0), Dict(1=>2, 3=>4))
1=>2
...
randstring(MersenneTwister(3), 'a':'z', 6)
"ocucay"
...
rng = Xoshiro(1234); rand(rng, 2) == x1
true

Xoshiro(1) == Random.seed!(rng, 1)
true

rand(Random.seed!(rng), Bool) # not reproducible
true

rand(Random.seed!(rng), Bool) # not reproducible either
false

rand(Xoshiro(), Bool) # not reproducible either
true
