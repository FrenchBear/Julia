...
inv(2//3)                   # 3//2
inv([1 2;3 4])              # 2×2 Matrix{Float64}: -2.0   1.0\n  1.5  -0.5
...
# Function ÷(x, y)
#
# The quotient from Euclidean (integer) division. Generally equivalent to a mathematical operation x/y without a
# fractional part.
...
# - if r == RoundNearest, then the result is in the interval [−𝜋,𝜋][−π,π]. This will generally be the most accurate
#   result. See also RoundNearest.
# - if r == RoundToZero, then the result is in the interval [0,2𝜋][0,2π] if x is positive,. or [−2𝜋,0][−2π,0]
#   otherwise. See also RoundToZero.
...
# ------------------------
# Base.fldmod Function fldmod(x, y)
#
# The floored quotient and modulus after division. A convenience wrapper for divrem(x, y, RoundDown). Equivalent to
# (fld(x, y), mod(x, y)).
...
#
# Approximate floating point number x as a Rational number with components of the given integer type. The result will
# differ from x by no more than tol.
...
# If length is not specified and stop - start is not an integer multiple of step, a range that ends before stop will be
# produced.
...
# The infix operation a ⊻ b is a synonym for xor(a,b), and ⊻ can be typed by tab-completing \xor or \veebar in the Julia
# REPL.
...
# 1×9 Matrix{Float64}:  0.0  0.707  1.0  0.707  0.0  -0.707  -1.0  -0.707  -0.0
...
asin(2.0+0im)               # 1.5707963267948966 + 1.3169578969248166im
...
factorial(6)                # 720
# factorial(21)             # ERROR: OverflowError: 21 is too large to look up in the table; consider using
