...
# I DONT'T UNDERSTAND HOW IT WORKS, IN MY TESTS, STR IS ALWAYS INTERPOLATED AT CONSTRUCTION TIME, NEVER AT PRINTING TIME...
...
collect(eachmatch(rx, "a1a2a3a", overlap = true))   # 3-element Vector{RegexMatch}: RegexMatch("a1a") RegexMatch("a2a") RegexMatch("a3a")
...
Note: The examples below may be rendered differently on different systems. The comments indicate how they're supposed to be rendered
...
Note: startswith does not compile the anchoring into the regular expression, but instead passes the anchoring as match_option to PCRE. If compile time is amortized, occursin(r"^...", s) is faster than startswith(s, r"...").
...
Note: endswith does not compile the anchoring into the regular expression, but instead passes the anchoring as match_option to PCRE. If compile time is amortized, occursin(r"...$", s) is faster than endswith(s, r"...").
